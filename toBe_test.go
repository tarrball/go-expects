...
func TestToBeIntMismatchFails(t *testing.T) {
	actual, expected := 1, 2
	mock := getMock(t)
	mock.EXPECT().Errorf("Expected '%d' to equal '%d'", expected, actual)

	expectation := expectationInt{expected, expectation{mock}}

	expectation.toBe(actual)
}
...
func TestToBeStringMismatchFails(t *testing.T) {
	...
	mock := getMock(t)
	mock.EXPECT().Errorf("Expected '%s' to equal '%s'", expected, actual)
...
func TestToBeUint64MatchPasses(t *testing.T) {
	...
	expectation := expectationUint64{expected, expectation{t}}
...
func TestToBeUint64MismatchFails(t *testing.T) {
	...
	expectation := expectationUint64{expected, expectation{mock}}

	expectation.toBe(actual)
}

func getMock(t *testing.T) *mocks.MocktestContext {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	return mocks.NewMocktestContext(mockCtrl)
}
