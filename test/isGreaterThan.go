...
// IsGreaterThan expects the system under test value to be greater than the expected value
func (sut SUT) IsGreaterThan(expected interface{}) {
	if sut.actual == nil {
	...
	if !didPass {
		toBeGreaterThanFail(sut.testContext,
			sutType,
			sut.actual,
...
func toBeGreaterThanFail(t testContext, objType reflect.Type, sut interface{}, expected interface{}) {
	switch objType.Kind() {
	case reflect.Int:
		t.Errorf("Expected '%d' to be greater than '%d'.", sut, expected)
	case reflect.Int8:
		t.Errorf("Expected '%d' to be greater than '%d'.", sut, expected)
	case reflect.Int16:
		t.Errorf("Expected '%d' to be greater than '%d'.", sut, expected)
	...
	// 	t.Errorf("'%s' should was not greater than '%s'.", sut, expected)
	case reflect.String:
		t.Errorf("Expected '%s' to be greater than '%s'.", sut, expected)
	case reflect.Float32:
		t.Errorf("Expected '%f' to be greater than '%f'.", sut, expected)
	case reflect.Float64:
		t.Errorf("Expected '%f' to be greater than '%f'.", sut, expected)
	}
}
