...
func (actual SUT) ToBeGreaterThan(expected interface{}) {
	...
	if !didPass {
		toBeGreaterThanFail(actual.testContext,
			actualType,
			actual.value,
...
func toBeGreaterThanFail(t testContext, objType reflect.Type, actual interface{}, expected interface{}) {
	...
	// 	t.Errorf("'%s' should was not greater than '%s'", actual, expected)
	case reflect.String:
		t.Errorf("Expected '%s' to be greater than '%s'", actual, expected)
	case reflect.Float32:
		t.Errorf("Expected '%f' to be greater than '%f'", actual, expected)
	case reflect.Float64:
		t.Errorf("Expected '%f' to be greater than '%f'", actual, expected)
	}
}
